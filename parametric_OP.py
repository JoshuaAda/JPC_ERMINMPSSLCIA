...
# CLASSES
class parametricOP():
    def __init__(self, n_vars = 10, n_eq = 5, n_ineq = 5, obj_type = "quad", eps_fb = 1e-6, op_dict = None,N=20,N_r=1,dt=0.005,n_x=4,n_u=2):
    ...
    def batch_gen_x_random(self,n_batch):
        ...
        u_tilde = torch.tensor([[0.95],[1]],device=self.device).T*torch.rand(n_batch, self.n_u)+torch.tensor([[0.05],[0]],device=self.device).T#torch.tensor([0.05,1]).repeat(n_batch).reshape(n_batch,-1)##torch.rand(n_batch, self.N_r * self.n_u)#self.spread_to_scenarios(self.u_tilde, n_batch)
        x0=torch.tensor([[0.95],[0.95],[0.625],[0.625]],device=self.device).T*torch.rand(n_batch,self.n_x,device=self.device)+torch.tensor([[0.05],[0.05],[0.375],[0.375]],device=self.device).T#torch.tensor([0.4,0.25,134.14/140,130/140]).repeat(n_batch).reshape(n_batch,-1)##.repeat_interleave(len(self.c_scen)**self.N_r,dim=0)
        past_u=torch.tensor([[0.95],[1]],device=self.device).T*torch.cat((torch.rand(n_batch,1),torch.randint(0,2,(n_batch, 1))),axis=1)+torch.tensor([[0.05],[0]],device=self.device).T#torch.tensor([[0.95],[1]],device=self.device).T*torch.rand(n_batch, self.n_u)+torch.tensor([[0.05],[0]],device=self.device).T#torch.tensor([0.05,1]).repeat(n_batch).reshape(n_batch,-1)#
        x = torch.hstack([x0, c,rho_new,lam, u_tilde,past_u])
        return x
    ...
    def batch_gen_x_new_cas(self,n_batch,x0=torch.tensor([]),u0=torch.tensor([])):
        ...
        if x0.numel()==0:
            ...
            if self.multistage:
                x = np.hstack((np.array(x0.cpu()), c,np.array(u0.cpu())))
                x=x.reshape((-1,1))
            else:
    ...
    def _casadi_setup(self):
        ...
        if self.obj_type == "nonlinear":
            if self.multistage:
                ...
                x_sym=x[0:(self.n_x + self.n_p + self.n_u )]
                ...
                for k in range(1,self.num_scen**self.n_p):
                    x_sym = x[k * (
                                self.n_x +  self.n_p + self.n_u ):(k+1) * (
                                self.n_x +  self.n_p  + self.n_u)]
        ...
        if self.multistage:
            ...
            lb_eq = np.zeros((self.n_eq+2)*self.num_scen**self.n_p-2)
            ub_eq = np.zeros((self.n_eq+2)*self.num_scen**self.n_p-2)
    ...
    def sample_random_data_small(self,num_samples=1000,data_dir="./sampling"):
        ...
        for k in range(num_samples):
            ...
            u1=res['x'][self.n_x*self.N:self.n_x*self.N+1].full().reshape((-1,))
            u0=res['x'][(self.n_x+1)*self.N:self.N*(self.n_x+2)].full().reshape((-1,))
            u0=np.concatenate((u1,u0))
            if self.ca_solver_small.stats()['success']:
                x_opt.append(x_curr)
                u0_opt.append(u0)
    ...
    def ode_parallel_lin(self, X, U,x):
        ...
        C_a=X[:, 0].detach().squeeze() * 2
        C_b = X[:, 1].detach().squeeze() * 2
        T_R= X[:, 2].detach().squeeze() * 140
        T_K = X[:, 3].detach().squeeze() * 140
        F= U[:, 0].detach().squeeze() * 100
        Q_dot= -U[:, 1].detach().squeeze() * 8500
        c_a = X[:, 0].detach().squeeze() * 2
        c_b = X[:, 1].detach().squeeze() * 2
        t_R = X[:, 2].detach().squeeze() * 140
        t_K = X[:, 3].detach().squeeze() * 140
        ...
        beta = 1
        T_dif=T_R - T_K
        T_in=x[self.n_x+4].repeat_interleave(self.N).detach()*140#:self.n_x+self.N*self.n_p:self.n_p]*140
        C_A0=x[self.n_x+2].repeat_interleave(self.N).detach()*5.7#:self.n_x+self.N*self.n_p:self.n_p]*5.7
        M_k=x[self.n_x+3].repeat_interleave(self.N).detach()*6#:self.n_x+self.N*self.n_p:self.n_p]*6
        Alpha=x[self.n_x].repeat_interleave(self.N).detach()*1.05#:self.n_x+self.N*self.n_p:self.n_p]*1.05
        Beta=x[self.n_x+1].repeat_interleave(self.N).detach()*1.1#:self.n_x+self.N*self.n_p:self.n_p]*1.1
        H_R_ab=x[self.n_x+5].repeat_interleave(self.N).detach()*4.6#:self.n_x+self.N*self.n_p:self.n_p]*4.6
        H_R_bc=-x[self.n_x+6].repeat_interleave(self.N).detach()*12#:self.n_x+self.N*self.n_p:self.n_p]*12
        t_in = x[self.n_x + 4].repeat_interleave(self.N).detach() * 140  #:self.n_x+self.N*self.n_p:self.n_p]*140
        c_A0 = x[self.n_x + 2].repeat_interleave(self.N).detach() * 5.7  #:self.n_x+self.N*self.n_p:self.n_p]*5.7
        m_k = x[self.n_x + 3].repeat_interleave(self.N).detach() * 6  #:self.n_x+self.N*self.n_p:self.n_p]*6
        alpha = x[self.n_x].repeat_interleave(self.N).detach() * 1.05  #:self.n_x+self.N*self.n_p:self.n_p]*1.05
    ...
    def sim_rk4_parallel(self, X, U,x):
        ...
        return X_next
    def sim_rk4_parallel_lin(self, X, U,x):
        """Simulate the system using RK4 method, in parallel."""
        dt = self.dt
        k1 = self.ode_parallel_lin(X, U,x)
        k2 = self.ode_parallel_lin(X + dt / 2 * k1, U,x)
        k3 = self.ode_parallel_lin(X + dt / 2 * k2, U,x)
        k4 = self.ode_parallel_lin(X + dt * k3, U,x)
        X_next = X + dt / 6 * (k1 + 2 * k2 + 2 * k3 + k4)
        return X_next
    ...
    def eq_system_lin(self, X_traj, U_traj,x):
        """Compute the equality constraint related to the continuity of the system dynamics in a multiple shooting approach."""
        assert X_traj.shape[0] == self.N + 1  # x0, x1, x2, ..., xN
        assert U_traj.shape[0] == self.N  # u0, u1, ..., uN-1
        assert X_traj.shape[1] == self.n_x
        ...
        X_traj_prev = X_traj[:-1, :]  # x0, x1, x2, ..., xN-1
        X_traj_next = self.sim_rk4_parallel_lin(X_traj_prev, U_traj,x)  # x1+, x2+, ..., xN+
    ...
    def h_func_lin(self, w,x):
        ...
        # eq_1 = (self.eq_system(X_traj, U_traj_1)*(1-p_u)).reshape(-1)
        # eq_2= (self.eq_system(X_traj, U_traj_2)*p_u).reshape(-1)
        eq = self.eq_system_lin(X_traj, U_traj,x).reshape(-1)
        return eq  # eq_1+eq_2
